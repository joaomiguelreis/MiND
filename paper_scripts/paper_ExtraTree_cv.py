...
for i in range(ncells):
	...
	precision_3 = cross_val_score(ExtraTreesClassifier(max_depth=3), X, y, scoring = 'precision', cv=10)
	precision_6 = cross_val_score(ExtraTreesClassifier(max_depth=6), X, y, scoring = 'precision', cv=10)
	precision_9 = cross_val_score(ExtraTreesClassifier(max_depth=9), X, y, scoring = 'precision', cv=10)

	# Making predictions on the testing features set with recall metric
	recall_3 = cross_val_score(ExtraTreesClassifier(max_depth=3), X, y, scoring='recall', cv=10)
	recall_6 = cross_val_score(ExtraTreesClassifier(max_depth=6), X, y, scoring='recall', cv=10)
	recall_9 = cross_val_score(ExtraTreesClassifier(max_depth=9), X, y, scoring='recall', cv=10)
...
#plt.fill_between(abscissa, prec_error_6-prec_errstd_6, prec_error_6+prec_errstd_6,  # fills area according to standard deviation
...
#plt.fill_between(abscissa, rec_error_6-rec_errstd_6, rec_error_6+rec_errstd_6,  # fills area according to standard deviation
#        alpha=0.5,color='#e0e0e0')
...
for i in range(ncells):
	...
	# Making predictions on the testing features set with precision metric
	precision_3 = cross_val_score(ExtraTreesRegressor(max_depth=3), X, y, scoring = 'neg_mean_squared_error', cv=10)
	precision_6 = cross_val_score(ExtraTreesRegressor(max_depth=6), X, y, scoring = 'neg_mean_squared_error', cv=10)
	precision_9 = cross_val_score(ExtraTreesRegressor(max_depth=9), X, y, scoring = 'neg_mean_squared_error', cv=10)
